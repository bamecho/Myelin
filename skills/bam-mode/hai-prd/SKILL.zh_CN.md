...
| 主要需求是执行阶段、任务和验证 | `bam-mode` plan reference |
...
3. **设计范围。** 决定什么放进这份 PRD，什么放进另一份 PRD、design doc、执行 plan 或任务。不要捆绑无关的清理、平台工作或重构，除非它们直接影响产品行为或验收。
...
## PRD Necessity          — Need PRD：推荐 PRD / bam-mode plan / design doc / 任务 / 不单独产物
...
**bam-mode plan reference**，模块边界与权衡用 **hai-architecture**，值不值得做用 **hai-idea**，代码/bugfix/测试
