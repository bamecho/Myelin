...
- A **Handoff contract** (see Output). Lock product tokens the user or repo already decided; list remaining locked-class gaps under Open instead of inventing them. Set Skip for entity-model, codebase-design, and plan thickness.
...
**Handoff contract** (required when another skill, agent, or implementer will continue; omit only for pure judgment/triage with no handoff):

```md
## Handoff contract
### Locked decisions
- <names, store/compute, token/regex/order, warning codes, non-goals already decided>
### Open decisions
- <blockers only: Impact; Default; Owner>
### Skip
- entity-model: yes|no — <reason>
- codebase-design: yes|no — <reason>
- plan thickness: none|note|full — <reason> (default none when design taskable)
```

Locked-class items (field/CLI/warning names, store vs compute, token/regex/order, product non-goals) must sit under Locked or Open. Do not silently fill them to look complete. Full rules live in `bam-mode/references/handoff-contract.md` when that skill is present.
