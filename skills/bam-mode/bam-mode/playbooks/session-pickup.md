...
1. Locate the prior trail from a path or URL supplied by the user or active host, a transcript explicitly scoped to the current workspace, a resume note, or a pushed branch. Never search global agent-history directories or unrelated workspaces. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in an isolated worker when available, or in bounded local passes otherwise, and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
