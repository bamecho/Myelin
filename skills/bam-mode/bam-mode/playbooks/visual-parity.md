...
3. Migrate one component at a time. Each is an independent artifact, so parallelize only when the host can give each owner isolated writable state; otherwise run the components sequentially (the **principle-separate-before-serializing-shared-state** skill). Shared primitives migrate first as a blocking phase.
