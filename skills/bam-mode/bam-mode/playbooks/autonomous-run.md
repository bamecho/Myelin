...
2. Make the loop idempotent before choosing the wake mechanism. Apply the **principle-make-operations-idempotent** skill: rerunning an iteration after a crash, timeout, or duplicate wake should converge to the same state, not duplicate commits, comments, files, or external actions.
...
4. Each iteration makes the smallest change the evidence justifies, verifies it against the predicate, commits if it advanced, discards changes that didn't help. Belt-and-suspenders that "might help" gets reverted, not left to ride.
   Sequence the work via the **principle-sequence-verifiable-units** skill, verifying each unit before the next instead of batching checks at the end.
